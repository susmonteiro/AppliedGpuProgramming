...
const char *mykernel =
    "__kernel \n"
    "void k () \n"
    "{ int g_index = get_global_id(0); int g_index2 = get_global_id(1);  int g_index3 = get_global_id(2); \n"
    "int l_index = get_local_id(0); int l_index2 = get_local_id(1);  int l_index3 = get_local_id(2); \n"
    "int w_index = get_group_id(0); int w_index2 = get_group_id(1);  int w_index3 = get_group_id(2); \n"
    "printf(\"Hello World! My global id is (%dx%dx%d), work-item ID is (%dx%dx%d) and work-group ID is (%dx%dx%d). \\n\", g_index, g_index2, g_index3, l_index, l_index2, l_index3, w_index, w_index2, w_index3); } \n";
...
int main(int argc, char *argv)
{
  ...
  cl_int err = clGetPlatformIDs(1, NULL, &n_platform);
  CHK_ERROR(err);
